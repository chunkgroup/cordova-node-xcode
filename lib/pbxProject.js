...
/////////////////////////////////////////////////////
//                                                 //
//    Custom Keyboard Extension Installation       //
//                                                 //
/////////////////////////////////////////////////////
//    
// Usage:
//
// Add the target
//    pbxProject.addKeyboardExtensionTarget()
//
// Add files (swift files, images, frameworks)
//    pbxProject.addKeyboardExtensionFile()
//
/////////////////////////////////////////////////////


// ============================================================================
// addKeyboardExtensionTarget
// ============================================================================
//
// Usage:
//
//   First step in adding a keyboard extension.
//
// Args:
//
//   -------------------------------------------------------------------------
//   internalName
//   String; the name for the keyboard target and any of its related
//   folders/groups within the Xcode project
//   -------------------------------------------------------------------------
//   topLevelFolderName
//   String; Usually the name of the app, the top level navigator group
//   of the Xcode project.
//   -------------------------------------------------------------------------
//   keyboardBundleId
//   String; the bundle ID of the keyboard; must be created based on main
//   app's bundle ID, e.g. com.company.app and com.company.app.keyboard
//   -------------------------------------------------------------------------
//
// Returns:
//
//   String; the new group key, to be used when going on to add files
//   to the keyboard extension.
//
pbxProject.prototype.addKeyboardExtensionTarget = function(internalName, topLevelFolderName, keyboardBundleId) {
    
    // Try to create a group for the extension. If it can't be created,
    // exit.
    var groupPath = topLevelFolderName + "/" + internalName;
    var groupKey = this.createKeyboardExtensionGroup(internalName, groupPath);
    if(!groupKey) {
        return null;
    }
    ...
    var targetName = internalName.trim() + ".appex";
    var bundleName = internalName.trim().split(' ').join('-');

    if(!producttypeForTargettype(targetType)) {
        throw new Error("Target type invalid: " + targetType);
    }
    var frameworkSearchPath = '"' + groupPath + '"';

    var buildConfigurationsList = [{
        name: 'Debug', isa: 'XCBuildConfiguration',
        buildSettings: { FRAMEWORK_SEARCH_PATHS: frameworkSearchPath, ALWAYS_SEARCH_USER_PATHS: 'NO', CLANG_ANALYZER_NONNULL: 'YES', CLANG_CXX_LANGUAGE_STANDARD: '"gnu++0x"', CLANG_CXX_LIBRARY: '"libc++"', CLANG_ENABLE_MODULES: 'YES', COPY_PHASE_STRIP: 'NO', DEBUG_INFORMATION_FORMAT: 'dwarf', ENABLE_STRICT_OBJC_MSGSEND: 'YES', ENABLE_TESTABILITY: 'YES', GCC_C_LANGUAGE_STANDARD: 'gnu99', GCC_DYNAMIC_NO_PIC: 'NO', GCC_NO_COMMON_BLOCKS: 'YES', GCC_OPTIMIZATION_LEVEL: '0', GCC_PREPROCESSOR_DEFINITIONS: [ '"DEBUG=1"', '"$(inherited)"', ], GCC_WARN_64_TO_32_BIT_CONVERSION: 'YES', GCC_WARN_ABOUT_RETURN_TYPE: 'YES_ERROR', GCC_WARN_UNINITIALIZED_AUTOS: 'YES_AGGRESSIVE',
        INFOPLIST_FILE: '"' + groupPath + '/Info.plist' + '"', LD_RUNPATH_SEARCH_PATHS: '"$(inherited) @executable_path/Frameworks @executable_path/../../Frameworks"',
        IPHONEOS_DEPLOYMENT_TARGET: '10.0', MTL_ENABLE_DEBUG_INFO: 'YES', PRODUCT_BUNDLE_IDENTIFIER: keyboardBundleId, PRODUCT_NAME: '"' + internalName + '"', SKIP_INSTALL: 'YES' }
    }, {
        name: 'Release', isa: 'XCBuildConfiguration',
        buildSettings: { FRAMEWORK_SEARCH_PATHS: frameworkSearchPath, ALWAYS_SEARCH_USER_PATHS: 'NO', CLANG_ANALYZER_NONNULL: 'YES', CLANG_CXX_LANGUAGE_STANDARD: '"gnu++0x"', CLANG_CXX_LIBRARY: '"libc++"', CLANG_ENABLE_MODULES: 'YES', COPY_PHASE_STRIP: 'NO', DEBUG_INFORMATION_FORMAT: '"dwarf-with-dsym"', ENABLE_NS_ASSERTIONS: 'NO', ENABLE_STRICT_OBJC_MSGSEND: 'YES', GCC_C_LANGUAGE_STANDARD: 'gnu99', GCC_NO_COMMON_BLOCKS: 'YES',
        INFOPLIST_FILE: '"' + groupPath + '/Info.plist' + '"', LD_RUNPATH_SEARCH_PATHS: '"$(inherited) @executable_path/Frameworks @executable_path/../../Frameworks"',
        IPHONEOS_DEPLOYMENT_TARGET: '10.0', MTL_ENABLE_DEBUG_INFO: 'NO', PRODUCT_BUNDLE_IDENTIFIER: keyboardBundleId, PRODUCT_NAME: '"' + internalName + '"', SKIP_INSTALL: 'YES', VALIDATE_PRODUCT: 'YES' }
    }];
    var buildConfigurations = this.addXCConfigurationList(buildConfigurationsList, 'Release', 'Build configuration list for PBXNativeTarget "' + targetName + '"');
    ...
    var productFile = this.addProductFile(productName, {
    ...
    var targetObj = {
        uuid: targetUuid,
        pbxNativeTarget: {
            isa: 'PBXNativeTarget', name: '"' + targetName.replace('.appex', '') + '"',
            productName: '"' + targetName.replace('.appex', '') + '"', productReference: productFile.fileRef,
            productType: '"' + producttypeForTargettype(targetType) + '"', buildConfigurationList: buildConfigurations.uuid,
            buildPhases: [], buildRules: [], dependencies: []
        }
    };
    this.addToPbxNativeTargetSection(targetObj);

    // Add standard 'Embed App Extensions' step to main target
    ...
    // Add other build phases
    this.addBuildPhase([], 'PBXResourcesBuildPhase', 'Resources', targetUuid);
    this.addBuildPhase([], 'PBXSourcesBuildPhase', 'Sources', targetUuid);
    this.addBuildPhase([], 'PBXFrameworksBuildPhase', 'Frameworks', targetUuid);

    this.addToPbxProjectSection(targetObj);
    this.addTargetDependency(this.getFirstTarget().uuid, [targetObj.uuid]);
    this.pbxProjectSection()[this.getFirstProject()['uuid']]['attributes']['TargetAttributes'] = {};
    this.pbxProjectSection()[this.getFirstProject()['uuid']]['attributes']['TargetAttributes'][targetObj.uuid] = {
        CreatedOnToolsVersion: '8.0',
        ProvisioningStyle: 'Automatic'
    };

    targetObj.groupName = internalName;
    targetObj.groupPath = groupPath;
    targetObj.groupKey = groupKey;
...
//   -------------------------------------------------------------------------
//   fileInfo
//   Object containing the properties from the keyboard.xml, just like
//   plugin.xml, e.g. fileInfo.src, fileInfo.custom, fileInfo.embed
//   -------------------------------------------------------------------------
//   target
//   The target object returned by addKeyboardExtensionTarget
//   -------------------------------------------------------------------------
//
// Returns:
//
//   Boolean; true if the file was fully added, false if the file
//   wasn't added to its corresponding build phase successfully.
//
pbxProject.prototype.addKeyboardExtensionFile = function(fileInfo, target) {
    var filepath = fileInfo.src;
    var opt = {};
    // We don't specify opt.sign (Code Sign On Copy) here like we do in other framework handling,
    // as this was crashing Xcode. It could be because linked frameworks specifically in the
    // keyboard extension don't have a corresponding Embedded Binaries section in the
    // General settings. For linked frameworks in a keyboard extension, they must also be
    // linked and embedded in the main app's target, so we do that further down.
    if(fileInfo.filetype == "framework" && fileInfo.custom == "true") {
        opt.embed = true;
        opt.link = true;
    ...
    file.uuid = this.generateUuid();
    var groupObj = this.pbxGroupByName(target.groupName);
    var correctedPathFile = null;
    if(groupObj) {
       correctedPathFile = correctForPath(file, this, target.groupName);
    } else {
        console.log("Can't correct for path: group '" + target.groupName + "' doesn't exist.");
    }
    file.fileRef = this.generateUuid();
    this.addToPbxBuildFileSection(file);
    this.addToPbxFileReferenceSection(file);
    this.addToPbxGroup(file, target.groupKey);
    var addedSuccessfullyToBuildPhase = false;

    if(fileInfo.filetype == "source-file") {
        var buildPhaseObj = this.buildPhaseObject("PBXSourcesBuildPhase", "Sources", file.target);
        if(buildPhaseObj) {
           buildPhaseObj.files.push(pbxBuildPhaseObj(file));
           addedSuccessfullyToBuildPhase = true;
        }
    } else if(fileInfo.filetype == "framework") {
        var buildPhaseObj = this.buildPhaseObject("PBXFrameworksBuildPhase", "Frameworks", file.target);
        if(buildPhaseObj) {
           buildPhaseObj.files.push(pbxBuildPhaseObj(file));
           addedSuccessfullyToBuildPhase = true;
           // For keyboard extensions, custom frameworks - which would need to be added as embedded binaries -
           // must be done on the main project's target, and found via runpath_search_paths two levels above.
           this.addKeyboardExtensionFrameworkFully(file, filepath, target.groupPath, opt);
        }
    } else if(fileInfo.filetype == "resource-file") {
        var buildPhaseObj = this.buildPhaseObject("PBXResourcesBuildPhase", "Resources", file.target);
        if(buildPhaseObj) {
           buildPhaseObj.files.push(pbxBuildPhaseObj(file));
           addedSuccessfullyToBuildPhase = true;
        }
    ...
    if(!addedSuccessfullyToBuildPhase) {
        console.log("File not added successfully to any build phase.");
        return false;
    }
    return true;
}
...
// ============================================================================
// createKeyboardExtensionGroup
// ============================================================================
//
// Usage:
//
//   To be used only by addKeyboardExtensionTarget
//
// Args:
//
//   -------------------------------------------------------------------------
//   name
//   String, e.g. "MyKeyboard"
//   -------------------------------------------------------------------------
//   pathRelativeToXcodeProj
//   String, e.g. "MyProject/MyKeyboard"
//   -------------------------------------------------------------------------
//
// Returns:
//
...
//   to the keyboard extension.
//
pbxProject.prototype.createKeyboardExtensionGroup = function(name, pathRelativeToXcodeProj) {
    var customTemplateKey = this.findPBXGroupKey({ name: "CustomTemplate" });
    if(customTemplateKey) {
        var newGroupKey = this.pbxCreateGroup(name, pathRelativeToXcodeProj);
        this.addToPbxGroup(newGroupKey, customTemplateKey);
        return newGroupKey;
    }
    // Can't add stickers target - no CustomTemplate pbxgroup found
    return null;
}
...
// ============================================================================
// addKeyboardExtensionFrameworkFully
// ============================================================================
//
// Usage:
//
//   To be used only by addKeyboardExtensionFile
//
// Args:
//
//   -------------------------------------------------------------------------
//   file
//   An existing pbxFile object representing a '.framework' file
//   -------------------------------------------------------------------------
//   filepath
//   The path to the file as found in the 'src' attribute in the
//   keyboard.xml definition
//   -------------------------------------------------------------------------
//   groupPath
//   The path of the keyboard extension group, relative to the Xcode
//   project, e.g. "MyTopLevelProjectFolder/MyGestureKeyboard"
//   -------------------------------------------------------------------------
//   opt
//   The options object built up when the pbxFile (passed to arg 1)
//   was created
//   -------------------------------------------------------------------------
//
// Returns:
...
//   Nothing
//
pbxProject.prototype.addKeyboardExtensionFrameworkFully = function(file, filepath, groupPath, opt) {
    file.target = this.getFirstTarget().uuid;
    this.addToPbxBuildFileSection(file);
    this.addToPbxFileReferenceSection(file);
    ...
    if(opt.customFramework) {
        opt.sign = true;
        // Add to framework search paths. This is done in a custom way
        // because the build in method to do this assumes the framework is
        // at a certain path, and our frameworks here are being bundled
        // inside a single group/folder for the keyboard extension.
        var configurations = nonComments(this.pbxXCBuildConfigurationSection());
        for(var config in configurations) {
            var buildSettings = configurations[config].buildSettings;
            if(unquote(buildSettings['PRODUCT_NAME']) != this.productName)
                continue;
            var pathString = '"' + groupPath + '"';
            buildSettings['FRAMEWORK_SEARCH_PATHS'].push(pathString);
        }
        if(embed) {
            opt.embed = embed;
            var embeddedFile = new pbxFile(filepath, opt);
            embeddedFile.uuid = this.generateUuid();
            embeddedFile.fileRef = file.fileRef;
